...
let
  ...
  kyaBpCiDeploy = pkgs.writeShellScript "kya-bp-ci-deploy" ''
    ...
    for _ in $(seq 1 45); do
      if ${pkgs.curl}/bin/curl -sf --connect-timeout 5 --max-time 10 http://127.0.0.1:3001/healthz >/dev/null 2>&1; then
        app_ok=1
        break
      fi
      sleep 2
...
in
{
  ...
  services.github-runners.kya-bp = {
    ...
    extraLabels = [ "kya-bp" ];
    workDir = "/var/lib/github-runner-work/kya-bp";
    serviceOverrides = {
      StateDirectory = lib.mkForce [
        "github-runner/kya-bp"
        "github-runner-work/kya-bp"
      ];
      BindPaths = lib.mkForce [ ];
      ExecPaths = [ "/var/lib/github-runner-work/kya-bp" ];
    };
    replace = true;
