...
let
  ...
  kyaCiDeploy = pkgs.writeShellScript "kya-fq-ci-deploy" ''
    ...
    for _ in $(seq 1 45); do
      if ${pkgs.curl}/bin/curl -sf --connect-timeout 5 --max-time 10 http://127.0.0.1:3003/healthz >/dev/null 2>&1; then
...
in
{
  ...
  services.github-runners.kya-fq = {
    ...
    extraLabels = [ "kya-fq" ];
    workDir = "/var/lib/github-runner-work/kya-fq";
    serviceOverrides = {
      StateDirectory = lib.mkForce [
        "github-runner/kya-fq"
        "github-runner-work/kya-fq"
      ];
      BindPaths = lib.mkForce [ ];
      ExecPaths = [ "/var/lib/github-runner-work/kya-fq" ];
    };
    replace = true;
