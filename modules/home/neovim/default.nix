...
}:
{
  ...
  home-manager.users.${username} = {
    # ./hm.nix is the shared (mobile-safe) config. The Claude Code integration
    # is desktop-only — added here, not in the shared set, so nix-on-droid/honor
    # stays lean and free of the CC plugin.
    imports = [
      ./hm.nix
